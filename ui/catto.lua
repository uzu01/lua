-- https://github.com/bloodball/UI-Librarys/blob/main/cattoware
...
library.theme = {
    ...
    accentcolor2 = Color3.fromRGB(0, 85, 255),
...
if uis.TouchEnabled and uis.KeyboardEnabled == false then
    ...
    Holder.Position = UDim2.new(0.100000001, 0, 0.100000001, 0)
    ...
    local OldSize = TextButton.Size
    local OldPos = TextButton.Position

    TextButton.MouseButton1Click:Connect(function()
        if uilib then
            uilib.Enabled = not uilib.Enabled
        end
        local First = Tween(TextButton, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {Size = UDim2.new(0,40,0,40), Position = UDim2.new(.25,5,.25,5),})
        local Last = Tween(TextButton, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {Size = OldSize, Position = OldPos,})	
...
function library:CreateWindow(name, size, hidebutton)
    ...
    local dragstart = function(input)
        if table.find(whitelistedinput, input.UserInputType) then
            dragging = true
    ...
    function window:CreateTab(name)
        ...
        function tab:CreateSector(name,side)
            ...
            function sector:AddButton(text, callback)
                ...
                button.Main.MouseButton1Click:Connect(button.callback)
                updateevent.Event:Connect(function(theme)
            ...
            function sector:AddToggle(text, default, callback, flag)
                ...
                function toggle:AddDropdown(items, default, multichoice, callback, flag)
                    ...
                    dropdown.Nav.MouseButton1Click:Connect(MouseButton1Click)
                    dropdown.BlackOutline2.MouseButton1Click:Connect(MouseButton1Click)
            ...
            function sector:AddKeybind(text,default,newkeycallback,callback,flag)
                ...
                keybind.Bind.MouseButton1Click:Connect(function()
                    keybind.Bind.Text = "[...]"
